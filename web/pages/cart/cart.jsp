	...
	<script type="text/javascript">
		$(function () {
			$("a.deleteItem").click(function () {
				return confirm("您是否要删除"+$(this).parent().parent().find("td:first").text());
			});
			/*修改数量*/
			$(".itemCount").change(function () {
				...
				if (confirm("您是否要更改《"+$(this).parent().prev().text()+"》的数量为:"+count)){
					//发起请求，给服务器保存修改
	...
	<div id="main">
		...
		<table>
			...
			<c:if test="${empty sessionScope.cart.items}">
				<tr>
					<td colspan="5">
						<a href="index.jsp">您还未添加任何商品，请添加</a>
					</td>
				</tr>
			</c:if>
			<%--如果购物车非空--%>
			...
			<c:forEach items="${sessionScope.cart.items}" var="entry">
				<tr>
					...
					<td>
						<input bookId="${entry.value.id}" class="itemCount" style="width: 50px" type="text" value="${entry.value.count}"/>
					</td>
